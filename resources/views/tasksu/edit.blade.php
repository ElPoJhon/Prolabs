    ...
    <div>
        <div class="max-w-4xl mx-auto py-10 sm:px-6 lg:px-8">
            <div class="mt-5 md:mt-0 md:col-span-2">
                <form method="post" action="{{ route('tasksu.update', $task->id) }}">
                    @csrf
                    @method('PUT')
                    <div class="shadow overflow-hidden sm:rounded-md">
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            <label for="country" class="block text-sm font-medium text-gray-700">Usuario</label>
                        <select name="user_id" id="user_id" autocomplete="user_id" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                            <option value="">-- SELECCIONE --</option>
                            @foreach($users as $user):?>
                                <option value="<?php echo $user->cedula; ?>" <?php if($user->cedula==$task->user_id){ echo "selected"; }?>><?php echo $user->cedula." - ".$user->name; ?></option>
                                @endforeach
